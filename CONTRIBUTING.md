...
빠르게 훑고 싶다면 [작업 흐름 체크리스트](/Users/eun/Documents/dev./code/frontend/certified-massage/docs/workflow.md)를 먼저 보세요.
...
2. 제목은 `Feature: ...`, `BugFix: ...`, `CI: ...`처럼 타입 접두어를 붙여 씁니다.
3. Issue에 목적, 범위, 검증 기준을 적습니다.
4. 생성 후 assignee와 타입에 맞는 label을 지정합니다.
5. 해당 이슈를 해결하는 브랜치를 팝니다.
6. PR 본문에 이슈 번호를 연결합니다.
...
```text
<type>(optional-scope): <제목>

<상세 설명 본문>
```

제목은 소문자 타입으로 시작하고 scope는 필요할 때만 붙입니다. 본문은 비워두지 않고 왜 바꿨는지 간단히 남깁니다. CI에서 `body-empty` 검사를 하므로 한 줄 이상 설명이 꼭 있어야 합니다. 관련 이슈가 이미 있을 때만 제목 끝이나 본문에 `#번호`를 함께 남깁니다.

본문은 한 줄이 너무 길지 않게 끊어 적는 것을 권장합니다. 로컬에서 확인하려면 루트에서 `npm install` 후 `npm run lint:commit`을 사용할 수 있습니다.
...
- PR 템플릿의 `Related Issue`, `Summary`, `Task Details`, `Validation`, `Review Requirements`를 채웁니다.
- `Related Issue 🧵` 섹션에는 반드시 `Closes #번호`를 넣습니다.
- 템플릿의 안내 문구와 `작업 1`, `작업 2` 같은 placeholder는 그대로 두지 않습니다.
- UI 변경이면 스크린샷을 첨부합니다.
